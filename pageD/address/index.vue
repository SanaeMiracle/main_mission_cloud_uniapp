        ...
        <block v-for="(item, index) in addresList" :key="index">
          <view class="address">
            <view class="personal" @tap="selAddrToOrder(item)">
              <view class="info-tit">
                <text class="name"
                  >{{ item.province }}{{ item.city }}{{ item.area }}</text
                >
              ...
              <view class="addr">
                <text class="addr-get"
                  >{{ item.receiver }} {{ item.mobile }}</text
                >
                <!-- <text class="addr-get"> {{item.province}}{{item.city}}{{item.area}}{{item.addr}}</text> -->
              </view>
            </view>
            <view class="select-btn">
              ...
              <view class="btns">
                <view class="enit" @click="toEditAddress(item.addrId)"
                  >编辑</view
                >
...
export default {
  ...
  methods: {
    ...
    list() {
      ...
      uni.request({
        // url: `https://wj.api.cestech.com.cn:48002/mission_b/p/address/list?userId=${userId}`,
        url: `https://wj.api.cestech.com.cn:48002/mission_b/p/address/list`,
        header: {
      ...
      uni.showModal({
        ...
        success: function (res) {
          ...
          if (res.confirm) {
            ...
            console.log(id)
            uni.request({
              // url: `https://wj.api.cestech.com.cn:48002/mission_b/p/address/deleteAddr?userId=${userId}&addrId=${id}`,
              url: `https://wj.api.cestech.com.cn:48002/mission_b/p/address/deleteAddr?addrId=${id}`,
              header: {
                'x-Resource-Token': uni.getStorageSync('xtoken'),
                //'x-Resource-Token':uni.getStorageSync('xtoken'), //自定义请求头信息
              },
    ...
    onDefaultAddr: function (e) {
      ...
      uni.request({
        // url: `https://wj.api.cestech.com.cn:48002/mission_b/p/address/defaultAddr?addrId=${addrId}&userId=${userId}`,
        url: `https://wj.api.cestech.com.cn:48002/mission_b/p/address/defaultAddr?addrId=${addrId}`,
        header: {
          'x-Resource-Token': uni.getStorageSync('xtoken'),
          //'x-Resource-Token':uni.getStorageSync('xtoken'), //自定义请求头信息
        },
