<template>
  <view>
    <view class="title-img">
      <image
        :src="listData.bkbActivityCustomization.coverUrl"
        mode="aspectFill"
    ...
    <view class="introduce">
      ...
      <view class="item"
        >活动名称：{{ listData.bkbActivityCustomization.activityName }}</view
      >
      <view class="item"
        >活动类型：{{ listData.bkbActivityCustomization.activityType }}</view
      >
    ...
    <view class="introduce">
      ...
      <view class="txt">{{
        listData.bkbActivityCustomization.activityIntro
      }}</view>
    </view>
    <view class="introduce">
...
import { activityPerson } from '@/common/api.js'
export default {
  data() {
    return {
      list_img: [],
  ...
  computed: {
    time() {
      if (
        this.listData.bkbActivityCustomization.startDate &&
        this.listData.bkbActivityCustomization.endDate
      ) {
        var difftime =
          (new Date(this.listData.bkbActivityCustomization.endDate) -
            new Date(this.listData.bkbActivityCustomization.startDate)) /
          1000 //计算时间差
  ...
  methods: {
    details() {
      uni.request({
        url: `https://wj.api.cestech.com.cn:48002/mission_b/wx/bkbActivityCustomization/queryById`,
        header: {
        ...
        success: res => {
          ...
          if (res.data.result.bkbActivityCustomization.activityType === 0) {
            ...
            res.data.result.bkbActivityCustomization.activityType === 1
          ) {
          ...
          res.data.result.bkbActivityCustomization.activityUrl
            .split(',')
            .map(v => {
              // v = url + v
              console.log(v, '图片')
