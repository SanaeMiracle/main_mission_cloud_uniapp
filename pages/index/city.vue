...
import { customerregions, customerallbyregion } from '@/common/api.js'
export default {
  components: {
    LeeSelectCity,
    selfPicker,
  ...
  methods: {
    ...
    useCurrentPosition({ selected, simple }) {
      this.submitHandler({
        selected,
        simple,
      })
          ...
          success: res => {
            ...
            getApp().globalData.qqmapsdk.reverseGeocoder({
              ...
              success: res => {
                ...
                const location = []
                // 寻找省
                const findProvince = this.cityDataTree.find(item => {
                  if (item.name == res.result.ad_info.province) {
                    return true
                  }
                  return false
                })
                if (findProvince) {
                  ...
                  //寻找市
                  const findCity = findProvince.children.find(item => {
                    if (item.name == res.result.ad_info.city) {
                      return true
                    }
                    return false
                  })
                  ...
                  if (findCity) {
                    ...
                    //寻找区
                    const findDistrict = findCity.children.find(item => {
                      if (item.name == res.result.ad_info.district) {
                        return true
                      }
                      return false
                    })
                ...
                if (location.length >= 2) {
                  if (
                    this.regions.indexOf(location[location.length - 1].value) >
                    -1
                  ) {
                    that.submitHandler({
                      selected: location,
                      simple: location.map(item => item.name),
                    })
                  } else {
                    uni.showToast({
                      title: '当前区域未开放,请手动选择',
