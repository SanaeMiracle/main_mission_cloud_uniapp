        ...
        <view class="main">
          ...
          <!-- <scroll-view scroll-y="true" class="leftmenu">
						<block v-for="(item, index) in categoryList" :key="index">
							<view class="menu-item" :class="{ red:changeRed == index}" :data-index="index"
								:data-id="item.categoryId" @click="change(item.categoryId,index)">
								{{item.categoryName}}
							</view>
                ...
                <view
                  ...
                  <view>
                    <image
                      ...
                      class="more-pic"
                      mode="scaleToFill"
                      style="width: 282rpx; height: 282rpx"></image>
                  </view>
                  <view class="prodName">{{ items.prodName }}</view>
                  <view class="point">兑换积分：{{ items.points }}</view>
                </view>
              </view>
            </view>
...
export default {
  ...
  methods: {
    ...
    categoryInfo() {
      uni.request({
        url: 'https://wj.api.cestech.com.cn:48002/mission_b/category/categoryInfo',
        header: {
        ...
        success: res => {
          ...
          res.data.result.map(item => {
            let _item = JSON.parse(
              JSON.stringify(item).replace('categoryName', 'name')
            )
            this.dataNew.push(_item)
          })
    ...
    pageprod_details() {
      uni.request({
        url: 'https://wj.api.cestech.com.cn:48002/mission_b/prod/pageProd',
        header: {
          'x-Resource-Token': uni.getStorageSync('xtoken'),
          //'x-Resource-Token':uni.getStorageSync('xtoken'), //自定义请求头信息
        },
      ...
      uni.request({
        url: 'https://wj.api.cestech.com.cn:48002/mission_b/prod/pageProdL',
        header: {
          'x-Resource-Token': uni.getStorageSync('xtoken'),
          //'x-Resource-Token':uni.getStorageSync('xtoken'), //自定义请求头信息
        },
    ...
    userinfo() {
      ...
      // 	uni.request({
      // 		url: `https://wj.api.cestech.com.cn:48002/mission_b/xcx_api/bkbAppUser/integralRecordSummation?phone=${phone}`,
      // 		method: "GET",
      // 		success: (res) => {
    ...
    insal() {
      uni.request({
        url: 'https://wj.api.cestech.com.cn:48002/mission_b/xcx_api/bkbChoiceqst_answer/mallTotalPoints',
