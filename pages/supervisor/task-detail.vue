<template>
  <view class="content u-p">
    ...
    <view v-if="!pageLoading">
      ...
      <view
        class="margin-top-lg"
        v-if="detail.Task.status != 3 && detail.Task.status !== ''">
        <u-button
          :disabled="detail.Task.status == 3"
          type="primary"
          :loading="btnLoading"
          :text="detail.Task.status | btnTextFilter"
          @click="submit()">
        </u-button>
      </view>
    </view>
...
<script>
import { taskone, taskaccept, tasksuccess } from '@/common/api.js'
...
export default {
  ...
  methods: {
    getDetail() {
      uni.request({
        url: 'https://wj.api.cestech.com.cn:48002/mission_b/v1/supervisor/task/one',
        header: {
        ...
        success: res => {
          ...
          if (res.data.result.Task.selectFloorUnitText) {
            res.data.select_floor_unit_arr =
              res.data.result.Task.selectFloorUnitText.split(',') || []
          } else {
            res.data.select_floor_unit_arr = []
          }
        ...
        uni.showModal({
          ...
          success: function (res) {
            if (res.confirm) {
              ...
              uni.request({
                url: 'https://wj.api.cestech.com.cn:48002/mission_b/v1/supervisor/task/accept',
                header: {
                  'x-Resource-Token': uni.getStorageSync('xtoken'),
                  //'x-Resource-Token':uni.getStorageSync('xtoken'), //自定义请求头信息
                },
        ...
        uni.showModal({
          ...
          success: function (res) {
            if (res.confirm) {
              ...
              uni.request({
                url: 'https://wj.api.cestech.com.cn:48002/mission_b/v1/supervisor/task/success',
                header: {
                  'x-Resource-Token': uni.getStorageSync('xtoken'),
                  //'x-Resource-Token':uni.getStorageSync('xtoken'), //自定义请求头信息
                },
