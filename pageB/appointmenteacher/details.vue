<template>
  <view>
    ...
    <view class="title">
      <view class="haed-img">
        <image
          :src="list.bkbTeacherManagement.headImage"
          mode="aspectFill"
          style="width: 100%; height: 100%">
        </image>
      </view>
      <view class="right">
        <view class="name">{{ list.bkbTeacherManagement.speechTeacher }} </view>
        <view class="">
          <span
            class="tab"
            v-if="list.bkbTeacherManagement.speechLevel == '志愿者'"
            style="padding: 0 10rpx; border-color: #3bcf99; color: #3bcf99"
            >{{ list.bkbTeacherManagement.speechLevel }}</span
          >
          <span
            class="tab"
            v-if="list.bkbTeacherManagement.speechLevel == '专家'"
            style="padding: 0 10rpx; border-color: #ff861b; color: #ff861b"
            >{{ list.bkbTeacherManagement.speechLevel }}</span
          >
          <span
            class="tab"
            v-if="list.bkbTeacherManagement.speechLevel == '普通老师'"
            style="padding: 0 10rpx; border-color: #1b71ff; color: #1b71ff"
            >{{ list.bkbTeacherManagement.speechLevel }}</span
          >
        </view>
        <view style="font-size: 30rpx"
          >{{ list.bkbTeacherManagement.speechYear }}年经验</view
        >
      </view>
    </view>
      ...
      <view class="list-img">
        <view class="img" v-for="item in list_img" :key="item">
          <image
            ...
            :src="item"
            mode="aspectFill"
            style="width: 660rpx; border-radius: 15rpx"></image>
        </view>
      </view>
    </view>
    <view class="btn">
...
<script>
const { dateTimePicker, getMonthDay } = require('@/common/dateTimePicker.js')
export default {
  data() {
    return {
      list_img: [],
  ...
  methods: {
    ...
    details() {
      uni.request({
        url: `https://wj.api.cestech.com.cn:48002/mission_b/wx/bkbAppointmentTeacher/queryById`,
        header: {
        ...
        success: res => {
          this.list_img = []
          let url =
            'https://wj.api.cestech.com.cn:48002/mission_b/sys/common/static/'
          res.data.result.bkbTeacherManagement.uploadPictures
            .split(',')
            .map(v => {
              // v = url + v
              console.log(v, '图片')
    ...
    reservation() {
      if (this.timeStr != '请选择') {
        ...
        uni.request({
          url: `https://wj.api.cestech.com.cn:48002/mission_b/wx/bkbAppointmentTeacher/activityPerson`,
          header: {
            'x-Resource-Token': uni.getStorageSync('xtoken'),
            //'x-Resource-Token':uni.getStorageSync('xtoken'), //自定义请求头信息
          },
