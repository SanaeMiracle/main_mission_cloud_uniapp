    ...
    <uni-card :extra="item.status | activityStatusText">
      <view
        ...
        <view style="display: flex; align-items: center">
          <image
            :src="item.bkbTrainingProgram.coverUrl"
            mode="aspectFit"
            style="width: 120rpx; height: 120rpx; margin-right: 10rpx"></image>
          <u--text size="16" :text="item.bkbTrainingProgram.title"></u--text>
        </view>
        <view v-if="item.bkbTrainingProgram.status == 0"> 待接受 </view>
        <view v-if="item.bkbTrainingProgram.status == 1"> 已接受 </view>
        <view v-if="item.bkbTrainingProgram.status == 2"> 进行中 </view>
        <view v-if="item.bkbTrainingProgram.status == 3"> 已完成 </view>
      </view>
      <view class="u-flex u-row-between">
        <view class="u-flex-1">
          <u--text
            size="13"
            :text="`开始时间：${item.bkbTrainingProgram.createTime}`">
          </u--text>
        </view>
        <view>
          <!-- <u--text size="13" :text="item.type|typeText">
          </u--text> -->
          <u--text size="13" :text="`预约人数：${item.count}`"> </u--text>
        </view>
        <!--     <view class="u-flex-1">
 ...
 -->
      ...
      <view slot="actions" class="card-actions">
        ...
        <view
          ...
          @click="accept(item)"
          v-if="item.bkbTrainingProgram.status == 0">
        ...
        <view
          class="card-actions-item"
          @click="start(item)"
          v-if="item.bkbTrainingProgram.status == 1">
        ...
        <view
          ...
          v-if="item.bkbTrainingProgram.status == 2">
        ...
        <view
          ...
          <u--text size="14" text="获取培训码"> </u--text>
        </view>
        ...
        <view
          ...
          v-if="item.bkbTrainingProgram.status == 3">
          <u--text size="14" text="查看结果"> </u--text>
        </view>
      </view>
    </uni-card>
    <u-modal
...
export default {
  ...
  methods: {
    ...
    startSubmit() {
      if (this.url_img) {
        uni.request({
          url: `https://wj.api.cestech.com.cn:48002/mission_b/wx/bkbTrainingTask/beginActivity`,
          header: {
            'x-Resource-Token': uni.getStorageSync('xtoken'),
            //'x-Resource-Token':uni.getStorageSync('xtoken'), //自定义请求头信息
          },
    ...
    acceptSubmit() {
      uni.request({
        url: `https://wj.api.cestech.com.cn:48002/mission_b/wx/bkbTrainingTask/acceptanceActivity`,
        header: {
          'x-Resource-Token': uni.getStorageSync('xtoken'),
          //'x-Resource-Token':uni.getStorageSync('xtoken'), //自定义请求头信息
        },
    ...
    // 		// })
    // 		uni.request({
    // 			url: `https://wj.api.cestech.com.cn:48002/mission_b/wx/WxApi/imgUpload`,
    // 			data: {
    // 				formData: {
    // 					file: file.url
    // 				}
