  ...
  <view class="box">
    <view class="header">
      ...
      <view class="box-right">
        ...
        <view class="left">
          <view
            >开始时间：{{
              item.bkbTrainingProgram.createTime.slice(0, 16)
            }}</view
          >
          <view style="width: 160rpx">{{ item.count | typeText }}</view>
        </view>
      </view>
    </view>
    <view class="card-actions">
      <view
        ...
        @click="goDetail(item)"
        v-if="
          item.bkbTrainingProgram.status == 2 ||
          item.bkbTrainingProgram.status == 1
        ">
        查看详情
      </view>
      <view
        ...
        v-if="
          (item.bkbTrainingProgram.status == 2 && item.quesStatus == '0') ||
          item.answerStatus == '1'
        ">
        进行评价
      </view>
      ...
      <view
        ...
        v-if="item.status === '1' && item.bkbTrainingProgram.status == 1">
        已预约
      </view>
      <view
        class="card-actions-item reservationbtn"
...
<script>
import { actStatus } from '@/common/enums/new_file.js'
...
export default {
  ...
  methods: {
    ...
    subscribeSubmit() {
      ...
      uni.request({
        url: `https://wj.api.cestech.com.cn:48002/mission_b/wx/bkbTrainingCourseReservation/appointment`,
        header: {
